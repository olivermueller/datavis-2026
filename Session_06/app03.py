# run this streamlit app: streamlit run app03.py
...
with tab1:
    ...
    # Display plot and catch its events
    fig2_event = st.plotly_chart(fig2, on_select="rerun")

    # Initialize clicked_league in session state if it doesn't exist
    if 'clicked_league' not in st.session_state:
        st.session_state.clicked_league = None

    # Update clicked_league if a bar is selected
    if len(fig2_event["selection"]["points"]) > 0:
        st.session_state.clicked_league = fig2_event["selection"]["points"][0]["label"]
    else:
        filtered_players_clicked = filtered_players
        st.session_state.clicked_league = None

    # Filter players if a league is selected
    if st.session_state.clicked_league:
        filtered_players_clicked = filtered_players[filtered_players['league'] == st.session_state.clicked_league]

    # Plotly scatter plot with xg on the x-axis and goals on the y-axis
    fig1 = px.scatter(filtered_players_clicked, x='xg', y='goals', color='primary_position',
                     hover_data=['team', 'league'],
                     hover_name='player')

    # 45 degree line
    max_value = max(filtered_players_clicked['xg'].max(), filtered_players_clicked['goals'].max())
    fig1.add_shape(
        type="line",
        x0=0, y0=0,
